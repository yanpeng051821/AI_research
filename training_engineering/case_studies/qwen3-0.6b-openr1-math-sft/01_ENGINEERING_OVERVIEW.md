...
正式结果之后已经完成 54 条固定分层样本的 GSM8K 人工错误分析，结论见
`analysis/gsm8k_target_capability/`。当前下一步不是立即追加训练；候选动作是用同一批样本做
512-token 解码预算敏感性实验，或继续做训练数据与通用回归样本的归因分析。任何新动作都
必须在新合同下独立记录，不能修改 256-token 的历史主结果。
